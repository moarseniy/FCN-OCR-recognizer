...
слева направо выбирает допустимые окна и нарезает ее на кропы `image_width`:
...
`crop_stride` задает желаемый шаг (по умолчанию равен `image_width`). Для каждой
номинальной позиции генератор ищет ближайшее допустимое положение в пределах
примерно половины шага. Проверяются обе границы, длина нормализованной разметки
и минимальный межсимвольный зазор. Размер кропа и масштаб текста сохраняются;
порядок остается слева направо, а соседние кропы могут перекрываться даже при
`crop_stride: image_width`. Одна X-позиция не выдается повторно.

Если в окрестности нет допустимого положения, это окно пропускается. Если вся
строка не дала кропов, выбирается новый вариант текста/шрифта/стиля (при
`render_text --text` сохраняется указанный текст). Число попыток ограничено.
...
Вероятность `ink_spacing_touch_probability` проверяется один раз для уже
выбранного окна с касающимися символами. При отказе окно пропускается; поиск
соседней позиции не используется для повторного броска случайности. Это
вероятность принятия такого окна, а не гарантированная доля касаний в датасете.

В конце генерации выводится `Crop planning`:

- `rendered_lines`: сколько длинных строк было отрисовано, включая неудачные попытки;
- `planned_crops`: сколько номинальных окон проверено;
- `accepted_crops`: сколько кропов принято;
- `shifted_crops`: сколько принятых кропов сдвинуто относительно номинальной позиции;
- `unavailable_slots`: сколько окон пропущено из-за отсутствия допустимой позиции;
- `touch_rejected`: сколько окон отклонено вероятностной проверкой касания.

Статистика относится ко всем отрисованным строкам. `accepted_crops` может быть
больше числа сохраненных примеров: последние кропы строки не сохраняются после
достижения заданного `samples` (при параллельной генерации также на границе чанка).
...
Терминал и подпись показывают индекс, число принятых кропов и их положение в
исходной длинной строке. В JSON рядом с картинкой сохраняются `crop_index`,
`crop_count`, `crop_bounds: [left, top, right, bottom]`, `source_size: [width, height]`
и `crop_statistics`. Правая и нижняя границы не включаются. Отсчет индексов
начинается с нуля; `--index -1` выбирает последний принятый кроп. Чтобы смотреть
разные кропы одной и той же строки, используйте одинаковый `--seed` во всех
запусках. Эти сведения добавляются при `--text`; чанки хранят изображения,
текст и таргеты без координат исходной строки.
