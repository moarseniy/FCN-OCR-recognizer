...
| `--baseline-line-pad` | Запас для строгого crop сверху и снизу как доля высоты между найденными линиями. `0.08` оставляет примерно 8% высоты строки с каждой стороны, `0` режет ровно по линиям. |
...
8. В строгом режиме crop строится после поворота только по паре верх/низ:
   ...
   симметричный запас относительно высоты между линиями, чтобы не резать края
   букв. Если после поворота пару линий найти не удалось, baseline crop не
