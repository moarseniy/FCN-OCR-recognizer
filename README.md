...
| `--baseline-top-pad` | Верхний запас для старого мягкого baseline crop. В строгом режиме crop идет прямо по линии; общий запас лучше задавать через `--y-pad`. |
| `--baseline-bottom-pad` | Нижний запас для старого мягкого baseline crop. В строгом режиме crop идет прямо по линии; общий запас лучше задавать через `--y-pad`. |
...
8. В строгом режиме crop строится после поворота только по паре верх/низ:
   ...
   расширения через bbox текста. Если после поворота пару линий найти не
   удалось, baseline crop не применяется. В мягком режиме остается прежний
   crop по линиям плюс bbox и `baseline_top_pad`/`baseline_bottom_pad`.
