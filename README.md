...
- опционально `dense_targets`, `binary_gap_targets` и `cut_projection_targets`
  для конкретных режимов обучения.
...
Для вертикального сегментатора есть `legacy_target_mode: binary_gaps`:
`final` имеет 2 выхода, а таргет содержит 0/1 по X-колонкам, где `1` означает
промежуток между символами. Для сегментатора в стиле cut-projection есть
...
`scale_x`, `y_pad` и `x_pad` — inference-only гиперпараметры предобработки.
`scale_x: 0.2` растягивает ширину на 20%, `scale_x: -0.2` сжимает на 20%.
`y_pad: 0.2` добавляет вертикальный паддинг перед resize, `y_pad: -0.2`
симметрично обрезает 20% высоты перед resize. `x_pad: 0.05` добавляет по 5%
текущей ширины слева и справа уже перед подачей в сеть.
