...
- FCN OCR: `task: fcn_ocr`
- вертикальный сегментатор разрезов: `task: vertical_segmentation`
- детектор верхней/нижней базовой линии: `task: baseline_detection`
...
- `vertical_segmentation_fcn` - легкая width-preserving сеть для вертикальной сегментации.
- `baseline_detection_fcn` - height/width-preserving сеть с выходом
  `B x 2 x H x W` для детекции верхней и нижней базовых линий.
- `residual_temporal_fcn` - более тяжелая width-preserving FCN с residual-блоками
  и dilated temporal convolutions по X; подходит и для OCR, и для вертикальной сегментации.
...
- `configs/train/eng_train_101_baseline_detection.yaml`
